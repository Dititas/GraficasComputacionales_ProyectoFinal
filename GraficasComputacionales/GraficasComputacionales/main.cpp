...
LRESULT CALLBACK WndProc(HWND hwnd, UINT umessage, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_KEYDOWN:
	{
		...
		case 0x43: // 'c':
		{
			m_Graphics->posicionCamara();
			stringstream ss;
			ss << "X: " << m_Graphics->XposF << " Y: " << m_Graphics->YposF << " Z: " << m_Graphics->ZposF;
			string tmp = ss.str();
			MessageBox(m_hwnd, tmp.c_str(), (LPCSTR)"Coordenadas Player/Camera", MB_OK);
		}
		break;
		case 0x4A: // 'J':
		{
		...
		case 0x50: // 'P':
		{
			if (m_Graphics->tercera)
			{
				m_Graphics->tercera = false;
			}
			else
			{
				m_Graphics->tercera = true;
			}
		}
		break;
		default:
